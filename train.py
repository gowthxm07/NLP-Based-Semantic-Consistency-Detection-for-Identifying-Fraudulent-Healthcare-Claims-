...
from sklearn.metrics import precision_recall_curve,roc_curve,accuracy_score,f1_score
...
print("=== CLAIM VERIFICATION NLI TRAINING ===")
...
def build_inputs(df):
    ...
    for _,row in df.iterrows():
        ...
        note_diags=row["diagnoses"]
        note_procs=row["procedures"]

        if isinstance(note_diags,str):
            note_diags=ast.literal_eval(note_diags)

        if isinstance(note_procs,str):
            note_procs=ast.literal_eval(note_procs)
...
"""
        ...
        hypothesis=row["claim_natural_text"]
